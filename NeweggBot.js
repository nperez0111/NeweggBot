...
try {
  ...
  console.error(
    "You did not specify your settings in config.json, refer to README.md for more details"
  );
...
async function report(log) {
  console.log(new Date().toString().split("G")[0] + ": " + log);
}
...
async function run() {
        ...
        try {
          ...
          try {
            ...
            break;
					...
					report("Manual authorization code required by Newegg, input it into the page now.");
      ...
      try {
        ...
        try {
          await page.waitForSelector(
            "#bodyArea > section > div > div > div.message.message-success.message-added > div > div.item-added.fix > div.item-added-info",
            { timeout: 500 }
          );
          break;
        } catch (err) {
          try {
            await page.waitForSelector(
              "#app > div.page-content > section > div > div > form > div.row-inner > div.row-body > div > div > div.item-container > div.item-qty > input",
              { timeout: 500 }
            );
            break;
          } catch (err) {
            try {
              await page.waitForSelector(
                "#bodyArea > div.article > form:nth-child(1) > table.shipping-group.subscription-group > tbody > tr > td:nth-child(3) > div > button:nth-child(2)",
                { timeout: 500 }
              );
              break;
            } catch (err) {}
  ...
  while (true) {
    try {
      // 'REMOVE ITEM(S)'
      await page.waitForSelector(
        "#app > div.page-content > div > div > div > div.modal-footer > button.btn.btn-secondary",
        { timeout: 500 }
      );
      await page.click(
        "#app > div.page-content > div > div > div > div.modal-footer > button.btn.btn-secondary",
    ...
    try {
      // at ShoppingItem url
      await page.waitForSelector(
        "#bodyArea > section > div > div > div.message.message-success.message-added > div > div.item-added.fix > div.item-operate > div > button.btn.btn-primary",
        { timeout: 500 }
      );
      await page.click(
        "#bodyArea > section > div > div > div.message.message-success.message-added > div > div.item-added.fix > div.item-operate > div > button.btn.btn-primary",
    ...
    try {
      await page.waitForSelector('[class="btn btn-primary btn-wide"]', {
        timeout: 500,
      });
      ...
      try {
        await page.waitForSelector(
          "#app > header > div.header2020-inner > div.header2020-right > div:nth-child(1) > div:nth-child(2) > a",
          { timeout: 500 }
        );
      } catch (err) {
        break;
    ...
    try {
      await page.waitForSelector(
        "#bodyArea > div.article > div.step-navigation > div.actions.l-right > div > a.button.button-primary.has-icon-right",
        { timeout: 500 }
      );
      await page.click(
        "#bodyArea > div.article > div.step-navigation > div.actions.l-right > div > a.button.button-primary.has-icon-right"
    ...
    try {
      await page.waitForSelector(
        '[class="button button-primary button-override has-icon-right"]',
        { timeout: 500 }
      );
      await page.click(
        '[class="button button-primary button-override has-icon-right"]'
  ...
  while (true) {
    ...
    try {
      await page.waitForSelector(
        "#orderSummaryPanelAndPayment > div > div.additional-info-groupbox > div > div > a",
        { timeout: 1000 }
      );
      await page.click(
        "#orderSummaryPanelAndPayment > div > div.additional-info-groupbox > div > div > a"
  ...
  while (true) {
    try {
      await page.waitForSelector(
        "#app > div > section > div > div > form > div.row-inner > div.row-body > div > div:nth-child(3) > div > div.checkout-step-body > div > div.checkout-tabs-wrap.margin-top > div.checkout-tab-content.is-active > div.item-cells-wrap.border-cells.tile-cells.three-cells.expulsion-one-cell.checkout-card-cells > div:nth-child(1) > div > label > div.retype-security-code > input",
        { timeout: 750 }
      );
      await page.type(
        "#app > div > section > div > div > form > div.row-inner > div.row-body > div > div:nth-child(3) > div > div.checkout-step-body > div > div.checkout-tabs-wrap.margin-top > div.checkout-tab-content.is-active > div.item-cells-wrap.border-cells.tile-cells.three-cells.expulsion-one-cell.checkout-card-cells > div:nth-child(1) > div > label > div.retype-security-code > input",
    ...
    try {
      await page.waitForSelector(
        "#app > div > section > div > div > form > div.row-inner > div.row-body > div.item-cells-wrap.tile-cells > div:nth-child(3) > div > div.checkout-step-body > div.checkout-step-done > div.card > div.retype-security-code > input",
        { timeout: 750 }
      );
      await page.type(
        "#app > div > section > div > div > form > div.row-inner > div.row-body > div.item-cells-wrap.tile-cells > div:nth-child(3) > div > div.checkout-step-body > div.checkout-step-done > div.card > div.retype-security-code > input",
  ...
  while (config.auto_submit == "true") {
    try {
      // incase of UPS suggested billing address
      await page.waitForSelector("#BillingForm > div > div.recommend > a", {
        timeout: 500,
      });
    ...
    try {
      // incase of you have to 'Accept terms'
